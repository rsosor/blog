 ...
 * @date 2021/9/2
 */
public interface IPropertyEnum extends IValueEnum<String> {
     ...
     */
    @SuppressWarnings("unchecked")
    static Object convertTo(@Nullable String value, @NonNull IPropertyEnum propertyEnum) {
    ...
    static Map<String, IPropertyEnum> getValuePropertyEnumMap() {
        // Get all properties
        List<Class<? extends IPropertyEnum>> propertyEnumClasses = new LinkedList<>();
        ...
        Map<String, IPropertyEnum> result = new HashMap<>();

        propertyEnumClasses.forEach(propertyEnumClass -> {
            IPropertyEnum[] propertyEnums = propertyEnumClass.getEnumConstants();

            for (IPropertyEnum propertyEnum : propertyEnums) {
    ...
    default <T> T defaultValue(Class<T> propertyType) {
        ...
        return IPropertyEnum.convertTo(defaultValue, propertyType);
    }
