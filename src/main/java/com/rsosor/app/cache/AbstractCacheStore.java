 ...
 */
@Slf4j
public abstract class AbstractCacheStore<K, V> implements ICacheStore<K, V> {

    protected RsosoRProperties rsosorProperties;
    ...
    public Optional<V> get(K key) {
        ...
        return getInternal(key).map(cacheWrapper -> {
            ...
            if (cacheWrapper.getExpireAt() != null
                && cacheWrapper.getExpireAt().before(com.rsosor.app.utils.DateUtils.now())) {
    ...
    private CacheWrapper<V> buildCacheWrapper(@NonNull V value, long timeout, @Nullable TimeUnit timeUnit) {
        ...
        Date now = com.rsosor.app.utils.DateUtils.now();
